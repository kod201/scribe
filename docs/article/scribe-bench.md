...
![Scribe pipeline: ingest, locate, read, validate, verify, then review queue or structured record](assets/pipeline.png)
...
![An untouched checkbox row: Specimen type — Blood, Urine, Stool, CSF, all boxes empty](assets/checkbox_trap_syn003.png)
...
3. **Generalization.** This one now has an answer. We ran the two best configurations against an external set nobody tuned against — [100 handwritten Indian prescriptions](https://huggingface.co/datasets/chaithanyakota/100-handwritten-medical-records) with the dataset author's own gold. Raw reading collapsed: brand names correct only 38% of the time on real doctors' cursive (`CEPODEM` → *Capelin*), versus ~80%+ on the volunteer-written flow set. But the safety architecture held: calibrated confidence dropped to 0.60–0.70 and **97–99% of pages routed to review**. Of the three pages that skipped the human, two were fully correct and one dropped a single medicine — one imperfect page escaped per hundred, under total distribution shift. The system cannot always read; it reliably knows when it can't. That's the property that was actually being engineered.
